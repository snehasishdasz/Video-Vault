  ...
  return (
    <Stack direction={["column","row"]} h={"100vh"}>
      <VStack w={'full'}>
        ...
        style={{width:"100%",borderRadius:"20px"}}></video>
      ...
      <VStack w={["full","xl"]} alignItems={"stretch"} p={10} spacing={8} overflow={"auto"} >

      
      {
        videosArr.map((item,index)=>(
          <Button w={"full"} color='#3F72AF' onClick={()=>setVideoSrc(item)} >Nature{index+1} </Button>
        ))
      }
      </VStack>
      <NavLink to="/aboutme">
